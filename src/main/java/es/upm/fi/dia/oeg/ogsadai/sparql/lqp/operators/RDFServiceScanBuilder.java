...
/**
 * executes the scan query extracted from the SERVICE definition
 * it does not build the actual query, it just executes it
 * 
 ...
 *
 */
public class RDFServiceScanBuilder implements ActivityPipelineBuilder
{
    /** Copyright statement. */
    ...
    {
        RDFServiceScanOperator operator = (RDFServiceScanOperator)op;
        
        GenericActivity query =
            new GenericActivity("es.upm.fi.dia.oeg.DirectSparqlEndpointQueryActivity");
        ...
        String endpointURL = operator.getEndpointURL();
        query.addInput("endpointURL", endpointURL);
        builder.add(query);
